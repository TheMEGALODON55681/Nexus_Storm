...
class DemoUIHelper:
    ...
    @staticmethod
    def get_article_card_UI_style(boarder_color="#4A90D9"):
        return {
            "card": {
                "width": "100%",
                "height": "116px",
...
def _display_main_article(
    ...
    if show_conversation and "conversation_log" in article_data:
        with st.expander(
            "Knowledge STORM uses a multi-perspective AI agent to research topics through simulated expert conversations.\n\n"
            ":mag: Click here to explore the AI research process"
        ):
...
class StreamlitCallbackHandler(BaseCallbackHandler):
    ...
    def on_identify_perspective_start(self, **kwargs):
        self.status_container.info(
            "Identifying research perspectives for your topic...."
        )

    def on_identify_perspective_end(self, perspectives: list[str], **kwargs):
        perspective_list = "\n- ".join(perspectives)
        self.status_container.success(
            f"Perspectives identified. Will now start gathering information"
            f" from the following perspectives:\n- {perspective_list}"
        )

    def on_information_gathering_start(self, **kwargs):
        self.status_container.info("Gathering information from the web....")
        ...
        for url in urls:
            self.status_container.markdown(
                f"""
                    ...
                    <div class="small-font">Processed source: <a href="{url}" class="small-font" target="_blank">{url}</a>.</div>
                    """,
                unsafe_allow_html=True,
            )

    def on_information_gathering_end(self, **kwargs):
        self.status_container.success("Information collection complete..")

    def on_information_organization_start(self, **kwargs):
        self.status_container.info(
            "Structuring collected knowledge into an outline...."
        )

    def on_direct_outline_generation_end(self, outline: str, **kwargs):
        self.status_container.success(
            f"Internal knowledge synthesis complete.."
        )

    def on_outline_refinement_end(self, outline: str, **kwargs):
        self.status_container.success(f"External knowledge synthesis complete..")
